...
- **Background Processing**: Reliable event delivery via polling mechanism
...
Events are persisted to the database and processed asynchronously by a background poller. The `emit()` method:
1. Persists the event to the outbox table atomically with any entity operations
2. Returns immediately after persistence
3. Event is picked up by the poller and delivered to listeners

This ensures reliable delivery even if the application crashes after persisting the event.
